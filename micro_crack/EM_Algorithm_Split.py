...
def micro_crack_estimator(datafile,ReportPath):
    ...
    for i in range(0, number_of_lot):                                 #原始數據良片生產率 [[-0.011505940186202065], [-0.00473117437590144]]
        ...
        if (tmp_calculation == 0):
            tmp_calculation = 0.001 # 0.001 --- 1.e-15
    ...
    return df, Config, same_machine, machine_name, number_of_rows, number_of_max_steps, \
            nonzero_foot_tmp, ng_piece, Recorded_goodpiece, machine_ini_prob, p, E_of_Zij, \
            bad_pieces_estimation_value
...
def e_step(Config, nonzero_foot_tmp, ng_piece, p):
    epsilon = 1.e-4
...
def m_step(number_of_max_steps, E_of_Zij, ng_piece, number_of_rows, Config, Recorded_goodpiece):
    ...
    return Mj_good_expect, Mj_bad_expect

def  EM_process (Config, same_machine, number_of_rows, number_of_max_steps, \
                 nonzero_foot_tmp, Recorded_goodpiece, ng_piece, opt_Pmj, p):    
    E_of_Zij = np.zeros(Config.shape)
    max_iteration = 1
    for _ in range(0, max_iteration):
        ...
        # E-Step Begin
        E_of_Zij = e_step(Config, nonzero_foot_tmp, ng_piece, p)
        # M-Step Begin
        Mj_good_expect, Mj_bad_expect = m_step(number_of_max_steps, E_of_Zij,    \
                                               ng_piece, number_of_rows, Config, \
                                               Recorded_goodpiece)
    ...
    print("stopped at:", _+1, "iterations")
    return p, E_of_Zij
...
def  print_report (p,bad_pieces_estimation_value,machine_name,ReportPath='bad_pieces_report.csv'):
        ...
        df = pd.read_csv(ReportPath, encoding='big5')
        print(df)
        return
...
def check_time(time_start, time_end):
    """
    calculate time passed from ``time_start`` to ``time_end``. the argument
    should be in ``float`` numbers or already in ``time`` format

    this function does not return anything, It prints inside function.
...
if __name__ == "__main__":
    ...
    raw_data, Config,  same_machine, machine_name, number_of_rows, number_of_max_steps, \
        nonzero_foot_tmp, ng_piece, Recorded_goodpiece, machine_ini_prob, p, E_of_Zij, \
        bad_pieces_estimation_value = micro_crack_estimator('test_case_1.csv',
                                                            ReportPath='bad_pieces_report.csv')

    #print the processing time
    check_time(start, time.time())